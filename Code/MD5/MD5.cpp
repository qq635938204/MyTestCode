...
inline CMD5::uint32 CMD5::RotateLeft(const uint32 x, int n)
{
	return (x << n) | (x >> (32 - n));
	// if x is signed, use: (x << n) | ((x & 0xFFFFFFFF) >> (32-n))
}
inline CMD5::uint32 CMD5::F(const uint32 x, const uint32 y, const uint32 z)
{
	return (x & y) | ((~x) & z);
}
inline CMD5::uint32 CMD5::G(const uint32 x, const uint32 y, const uint32 z)
{
	return (x & z) | (y & (~z));
}
inline CMD5::uint32 CMD5::H(const uint32 x, const uint32 y, const uint32 z)
{
	return x ^ y ^ z;
}
inline CMD5::uint32 CMD5::I(const uint32 x, const uint32 y, const uint32 z)
{
	return y ^ (x | (~z));
}

inline void CMD5::FF(uint32 &a, const uint32 b, const uint32 c, const uint32 d,
	const uint32 Mj, const int s, const uint32 ti)
{
	a = b + RotateLeft(a + F(b, c, d) + Mj + ti, s);
}
inline void CMD5::GG(uint32 &a, const uint32 b, const uint32 c, const uint32 d,
	const uint32 Mj, const int s, const uint32 ti)
{
	a = b + RotateLeft(a + G(b, c, d) + Mj + ti, s);
}
inline void CMD5::HH(uint32 &a, const uint32 b, const uint32 c, const uint32 d,
	const uint32 Mj, const int s, const uint32 ti)
{
	a = b + RotateLeft(a + H(b, c, d) + Mj + ti, s);
}
inline void CMD5::II(uint32 &a, const uint32 b, const uint32 c, const uint32 d,
	const uint32 Mj, const int s, const uint32 ti)
{
...
void CMD5::LinkResult()
{
	...
	for (int i = 0; i < 16; i++)
		sprintf_s(&md5Result_hex_[i * 2], 33,"%02x", md5Result_[i]);
	md5Result_hex_[32] = '\0';
...
void CMD5::printMd5()
{
	...
	{
#ifdef _DEBUG
		OutputDebugStringA("Error: computation is not finished\r\n");
#endif
	}
	else
	{
#ifdef _DEBUG
		OutputDebugStringA("CMD5 Value: ");
		OutputDebugStringA(md5Result_hex_);
		OutputDebugStringA("\r\n");
#endif
	}
}
...
string CMD5::GetMd5()
{
	...
	if (!isDone_)
	{
		...
		str = ((const char *)md5Result_hex_);
	return str;
}

void CMD5::UcharToUint(uint32 output[], const uchar8 input[], const unsigned int transLength)
{
...
void CMD5::FourRound(const uchar8 block[])
{
...
void CMD5::UpdateMd5(const uchar8 input[], const int length)
{
	isDone_ = false;
	...
	if (start <= length)
	{
		...
		memcpy_s(&remain_[remainNum_], 64,input, start);
		FourRound(remain_);
		...
		remainNum_ = length - i;
		memcpy_s(remain_, 64,&input[i],remainNum_);
	}
	else
	{
		// can not form a block, function return;
		memcpy_s(&remain_[remainNum_], 64,input, length);
		remainNum_ += length;
	}

}

void CMD5::UpdateMd5(const char8 input[], const int length)
{
...
void CMD5::ComputMd5(const uchar8 input[], const int length)
{
	init();
...
void CMD5::ComputMd5(const char8 input[], const int length)
{
